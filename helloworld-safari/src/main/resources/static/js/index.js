...
const ws = new WebSocket('wss://' + location.host + '/helloworld-safari');
...
function makePeer(sdpOffer, webRtcEpId)
{
      ...
      function(err)
  {
    ...
    console.log("[WebRtcPeer] Created; start local video");
    ...
    peer.generateOffer((err, sdpOffer) => {
      if (err) {
        sendError("[WebRtcPeer.generateOffer] Error: " + err);
        stop();
        return;
      }
      ...
      reportTimeoutId = window.setTimeout(() => {
        ...
        console.log("[WebRtcPeer.generateOffer] Generate User Report");
      ...
      console.log("[WebRtcPeer.generateOffer] Done!");
